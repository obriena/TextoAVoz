...
do
  ...
  if [[ ${REASON} == *ErrImagePull* ]] || [[ ${REASON} == *ImagePullBackOff* ]]; then
    ...
    echo "Application is unable to start, check the com.flyingspheres.services.application startup logs"
    break; # no need to wait longer, error is fatal
  fi
  sleep 5
done
...
echo -e "View the com.flyingspheres.services.application health at: http://${IP_ADDR}:${PORT}/health"
